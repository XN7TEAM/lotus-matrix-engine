...
def generate_threat_codename() -> str:
...
def handle_api_request():
    ...
    try:
        ...
        return jsonify({"success": False,
                        "error": "Internal core structural failure."}), 500
...
def get_engine_telemetry():
    """Real-time live cylinder plane + governor telemetry from the core engine."""
    live = spatial_engine.get_live_telemetry()
    if system_telemetry["attack_active"]:
        live["system_status"] = "UNDER ATTACK"
        live["governor_apex_rad"] = round(random.uniform(2.500, 3.999), 3)
    else:
        live["system_status"] = "SECURE"
    return jsonify(live)
...
def simulate_attack():
    ...
    if attack_state:
        ...
        sim_ip   = (f"185.{random.randint(10,254)}."
